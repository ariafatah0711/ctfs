...
-- Users table assumed sudah ada dengan kolom: id, email, is_admin, dst.
...
CREATE TABLE IF NOT EXISTS public.challenges (
  ...
  flag TEXT NOT NULL,             -- plain flag (hanya admin yang boleh lihat)
  flag_hash TEXT UNIQUE NOT NULL, -- hash untuk validasi
  hint TEXT,
...
-- Safe view (tanpa flag)
CREATE OR REPLACE VIEW public.challenges_safe AS
SELECT
  id,
  title,
  description,
...
-- 3. REVOKE & GRANT
-- ==============================================
-- Pastikan anon tidak punya akses apapun
REVOKE ALL ON ALL TABLES IN SCHEMA public FROM anon;
REVOKE ALL ON ALL SEQUENCES IN SCHEMA public FROM anon;
REVOKE ALL ON ALL FUNCTIONS IN SCHEMA public FROM anon;

-- Authenticated minimal access
GRANT SELECT, INSERT, UPDATE ON public.users TO authenticated;
GRANT SELECT ON public.challenges TO authenticated;
GRANT SELECT, INSERT ON public.solves TO authenticated;

-- Supaya authenticated bisa cek helper function (misal is_admin)
GRANT EXECUTE ON FUNCTION is_admin() TO authenticated;
GRANT EXECUTE ON FUNCTION is_admin(UUID) TO authenticated;

-- Authenticated boleh baca safe view
GRANT SELECT ON public.challenges_safe TO authenticated;
...
-- SELECT: hanya admin boleh lihat challenges (flag asli)
