...
const STORAGE_KEY_PREFIX = 'ctfs_sub_challenge_answers_v1:'
...
export function SubChallengesProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = async (challengeId: string, pAnswers?: SubChallengeAnswerMap): Promise<SubChallengeState> => {
    ...
    const response = await getSubChallenges(challengeId, currentAnswers)
  ...
  const submit = async (challengeId: string, orderNumber?: number): Promise<SubmitSubChallengesResult> => {
    setStates((prev) => ({
      ...prev,
      [challengeId]: {
