...
export default function DevConfigDialog({ open, onOpenChange }: DevConfigDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn(DIALOG_CONTENT_CLASS_3XL, "max-h-[90vh] overflow-hidden flex flex-col p-0 border border-gray-200 dark:border-gray-800")}>
        {/* Header */}
              ...
              <Section title="Identity & Branding" description="Core platform naming and flag rules.">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="lg:col-span-1">
                    ...
                    <Input value={config.shortName} onChange={(e) => updateField('shortName', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4"} />
                  </div>
                  <div className="lg:col-span-3">
                    ...
                    <Input value={config.fullName} onChange={(e) => updateField('fullName', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4"} />
                  </div>
                  <div className="lg:col-span-2">
                    ...
                    <Input value={config.flagFormat} onChange={(e) => updateField('flagFormat', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-blue-500 dark:text-blue-400"} />
                  </div>
                  <div className="lg:col-span-2">
                    ...
                    <Input value={config.description} onChange={(e) => updateField('description', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4"} />
                  </div>
                </div>
              </Section>
                ...
                <div className="space-y-6">
                  <div className="flex gap-2">
                    <Input
                      ...
                      className={SURFACE_GLASS_INPUT_CLASS + " h-11 px-4"}
                  ...
                  <div className="grid gap-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <Input value={config.eventMainLabel} onChange={(e) => updateField('eventMainLabel', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4"} />
                      </div>
                      <div>
                        ...
                        <Input value={config.discord} onChange={(e) => updateField('discord', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-[11px]"} />
                      </div>
                    </div>
                    <div>
                      ...
                      <Input value={config.eventMainImageUrl} onChange={(e) => updateField('eventMainImageUrl', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-[11px]"} />
                    </div>
                    <div>
                      ...
                      <Input value={config.eventFallbackImageUrl} onChange={(e) => updateField('eventFallbackImageUrl', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-[11px]"} />
                    </div>
                  </div>
                </div>
              </Section>
              ...
              <Section title="Database Engine" description="Supabase connection credentials.">
                <div className="space-y-6">
                  <div>
                    ...
                    <Input value={secret.supabaseUrl} onChange={(e) => updateSecretField('supabaseUrl', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-[11px]"} />
                  </div>
                  <div>
                    ...
                    <Input value={secret.supabaseAnonKey} onChange={(e) => updateSecretField('supabaseAnonKey', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-[11px]"} />
                  </div>
                </div>
              </Section>
                ...
                <div className="space-y-6">
                  ...
                  <div className={cn("grid gap-6 transition-all duration-300", !secret.nxctlEnabled && "opacity-40 pointer-events-none scale-[0.98]")}>
                    <div>
                      ...
                      <Input value={secret.nxctlApiUrl} onChange={(e) => updateSecretField('nxctlApiUrl', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-[11px]"} />
                    </div>
                    <div>
              ...
              <Section title="Security" description="Captcha and anti-bot protection.">
                <div className="space-y-6">
                  ...
                  <div className={cn("transition-all duration-300", !secret.turnstileSiteKeyEnabled && "opacity-40 pointer-events-none scale-[0.98]")}>
                    ...
                    <Input value={secret.turnstileSiteKey} onChange={(e) => updateSecretField('turnstileSiteKey', e.target.value)} className={SURFACE_GLASS_INPUT_CLASS + " h-10 px-4 font-mono text-[11px]"} />
                  </div>
                </div>
              </Section>
            </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-1">
        <h4 className={cn(TYPO_SECTION_TITLE_CLASS, "text-blue-500 dark:text-blue-400 flex items-center gap-2")}>
          <span className="w-1.5 h-1.5 rounded-full bg-blue-500/50" />
          {title}
        </h4>
        <p className={TYPO_MUTED_CLASS}>{description}</p>
      </div>
